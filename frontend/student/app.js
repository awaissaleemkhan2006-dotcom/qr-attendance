/**
 * Student Interface — Application Logic
...
async function loadCourses() {
    try {
        const res = await fetch(`${FLASK_API}/attendance/courses`, {
...
async function loadRecentAttendance() {
    try {
        const res = await fetch(`${FLASK_API}/attendance/history`, {
            headers: { 'Authorization': `Bearer ${authToken}` }
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error);
...
async function submitAttendance(qrData) {
    ...
    try {
        ...
        const res = await fetch(`${FLASK_API}/attendance/mark`, {
            method: 'POST',
            headers: {
