/**
 * Teacher Interface — Application Logic
 * Handles: Login, Dashboard, Session lifecycle, WebSocket QR refresh
 */

// ─── Configuration ────────────────────────────────────────────
const FLASK_API = 'http://localhost:5000/api';
const EXPRESS_API = 'http://localhost:3000/api';
...
async function loadCourses() {
    try {
        const res = await fetch(`${FLASK_API}/attendance/teacher/courses`, {
...
async function loadSessions() {
    try {
        const res = await fetch(`${FLASK_API}/attendance/teacher/sessions`, {
            headers: { 'Authorization': `Bearer ${authToken}` }
        });
        const data = await res.json();
...
async function handleEnrollStudent(e) {
    ...
    try {
        const res = await fetch(`${FLASK_API}/attendance/teacher/enroll`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`
...
async function startSession(courseId, courseName) {
    try {
        const res = await fetch(`${EXPRESS_API}/sessions/start`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`
        ...
        // Display initial QR
        if (data.session.qr) {
            updateQRDisplay(data.session.qr.qrDataUrl);
        }

        // Connect WebSocket for live QR updates
...
async function handleStopSession() {
    ...
    try {
        const res = await fetch(`${EXPRESS_API}/sessions/stop`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`
        ...
        showToast(`Session ended. ${data.session.total_attendance} students marked present.`, 'success');
...
// ─── WebSocket for Live QR ────────────────────────────────────
function connectWebSocket(sessionId) {
    disconnectWebSocket();

    wsConnection = new WebSocket(`${WS_URL}?sessionId=${sessionId}`);

    wsConnection.onopen = () => {
        console.log('WebSocket connected for QR updates');
    };

    wsConnection.onmessage = (event) => {
        const msg = JSON.parse(event.data);

        if (msg.type === 'qr_update') {
            updateQRDisplay(msg.qrDataUrl);
            startTimer(msg.refreshIn || 10);
...
function startAttendancePolling(sessionId) {
    ...
    const poll = async () => {
        try {
            const res = await fetch(`${EXPRESS_API}/sessions/${sessionId}/attendance`, {
                headers: { 'Authorization': `Bearer ${authToken}` }
            });
            const data = await res.json();
...
async function viewSessionAttendance(sessionId, courseName) {
    ...
    try {
        const res = await fetch(`${FLASK_API}/attendance/teacher/sessions/${sessionId}/attendance`, {
            headers: { 'Authorization': `Bearer ${authToken}` }
        });
        const data = await res.json();
